...
console.log(store);
 ...
 <template>
	<div>
		<div class="px-4 pt-4">
			<div class="border shadow-lg rounded-2xl max-w-sm">
				<div class="flex justify-between w-full bg-gray-200 rounded-2xl">
					<div class="flex flex-col px-3 py-2 space-y-2">
						<p class="p-1 px-2 rounded-full bg-pink-500 text-sm font-bold text-white">{{ store.storeHours?.date ?? 'Sin datos' }}</p>
						<!-- retiro o delivery -->
						<p class="p-1 px-2 rounded-full bg-pink-200 text-sm font-bold text-pink-500">{{ store.storeHours?.pickup_available ?? 'Sin datos' }}</p>
					</div>
					<div class="pr-6 pt-2">
